...
if(hplp+feerv/3<cp && hplptrend=='L'){
fs.writeFile('localStorage_data/hbardata.json', JSON.stringify({hplp:cp, hplptrend:hplptrend, otb:otb}), function(err){if(err)throw err;});
}; if(cp<hplp-feerv/3 && hplptrend=='S'){
fs.writeFile('localStorage_data/hbardata.json', JSON.stringify({hplp:cp, hplptrend:hplptrend, otb:otb}), function(err){if(err)throw err;});
}; if(hplp+dourv<cp && hplptrend=='S'){
fs.writeFile('localStorage_data/hbardata.json', JSON.stringify({hplp:cp, hplptrend:'L', otb:OS}), function(err){if(err)throw err;});
}; if(cp<hplp-dourv && hplptrend=='L'){
fs.writeFile('localStorage_data/hbardata.json', JSON.stringify({hplp:cp, hplptrend:'S', otb:'OL'}), function(err){if(err)throw err;});
}; console.log(coin+cp+"   lep"+lep+" lamt"+lamt+" sep"+sep+" samt"+samt+" "+hplptrend+hplp+" "+otb);};ourRequest.send();});});}, 10000);};funhbar();
